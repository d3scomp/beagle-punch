...
#define snprintf_loc(buf, size, fmt, args...) snprintf(buf, size, fmt,##args)
#define EINVAL 1

static int kstrtoint(const char * input, int base, int * ptr)
{
	char *retval;
	*ptr = strtol(input, &retval, base);
	if (*input != 0 && *retval == 0)
	{
		return 0;
	}
...
int json_print_status(char * buf, size_t size, struct pp_t * pp_sim)
{
	...
	return snprintf_loc(
		...
		//(int)pp_sim->x_axis.power, (int)pp_sim->y_axis.power,
		pp_fail(pp_sim) ? truestr : falsestr,
		pp_sim->last_punch.x_pos, pp_sim->last_punch.y_pos,
		//(int)pp_sim->x_axis.encoder, (int)pp_sim->y_axis.encoder,
		//(int)pp_sim->x_axis.errors, (int)pp_sim->y_axis.errors,
		//pp_sim->remaining_punch_time,
...
int json_apply_command(struct pp_t * pp_sim, const char * json_command)
{
	...
	command[0] = 0;
	// printk(KERN_ALERT "received: %s", json);
	while ((json_command = get_json_item(&item, json_command)) != 0)
	{
		// printk(KERN_ALERT "item: name: %s, value: %s, rem: %s\n", item.name, item.value, json);
	...
	// printk(KERN_ALERT "PPSIM: command: %s, x: %d, y: %d\n", command, x, y);
